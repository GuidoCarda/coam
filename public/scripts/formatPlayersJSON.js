...
players.forEach((team, teamIndex) => {
  ...
  team.players.forEach((player, playerIndex) => {
    player.image = `https://copaamerica.com/_next/image?url=%2Fplayers%2F${
      TEAMS[teamIndex]
    }%2F${TEAMS[teamIndex]}-${
      playerIndex < 9 ? "0" + (playerIndex + 1) : playerIndex + 1
    }.png&w=384&q=75`;
    player.id = `${team.id}-${player.name.toLowerCase().replace(" ", "")}-${
      playerIndex + 1
    }`;
  });
});
